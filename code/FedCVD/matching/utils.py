...
try:
    ...
    _LAPSOLVER_AVAILABLE = True
except Exception:
    _LAPSOLVER_AVAILABLE = False
    def solve_dense(cost):
        cost = np.asarray(cost)
        row_ind, col_ind = linear_sum_assignment(cost)
        # return a dense assignment array: for each row i, assign[i] = column index or -1
        assign = -np.ones(cost.shape[0], dtype=int)
